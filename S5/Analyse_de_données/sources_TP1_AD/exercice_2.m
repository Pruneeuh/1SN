...
[C,bornes_C,vecteur_RVB2gris_normalise] = ACP(X); % FONCTION A CODER
C1 = C(:,1);
C2 = C(:,2);
C3 = C(:,3);
...
subplot(7,3,11);
axis off
title(['r_{C_1C_2} = ' num2str(correlation(1,2),'%.2f')],'FontSize',20);

% Affichage des correlations :
subplot(7,3,14);
axis off
title(['r_{C_1C_3} = ' num2str(correlation(1,3),'%.2f')],'FontSize',20);

% Affichage des correlations :
subplot(7,3,17);
axis off
title(['r_{C_2C_3} = ' num2str(correlation(2,3),'%.2f')],'FontSize',20);
