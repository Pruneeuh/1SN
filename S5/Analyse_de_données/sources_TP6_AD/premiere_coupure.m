...
function [valeur_coupure,ind_variable_coupure] = premiere_coupure(X,Y)
    ...
    for k = 1:size(X,2)
        ...
        for ind_decoupe = 1:size(X,1)-1
            ...
            i_g = 1 - (sum(Y_g ==1)/size(Y_tries,1))^2 - (sum(Y_g ==-1)/size(Y_tries,1))^2; 
            i_d = 1 - (sum(Y_d ==1)/size(Y_tries,1))^2 - (sum(Y_d ==-1)/size(Y_tries,1))^2; 

            % Calcul du delta_i_modifie = P_g * i_g + P_d * i_d
            P_g = size(Y_g,1)/size(Y_tries,1);
            P_d = size(Y_d,1)/size(Y_tries,1);
