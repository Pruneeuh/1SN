...
procedure Evaluer_Alea is
    ...
    procedure Calculer_Statistiques (
        ...
        Min, Max : in out Integer; -- min et max des fréquences de l'échantillon
    ...
    is
        ...
        Capacite: constant Integer := Borne;   -- Cette taille est arbitraire
    ...
    type T_Tableau is
     ...
     Occurences : T_Tableau; -- Tableau des occurences de chaque valeur    
     Nombre_alea : Integer ;     -- Valeur générée aléatoirement
     

    begin
        -- initialisation du tableau des occurences
        for k in 1..Borne loop 
            Occurences.Elements(k) := 0; 
        end loop; 
        
        -- générer les valeurs aléatoires
        for i in 1..Taille loop
            Get_Random_Number(Nombre_alea); 
            Occurences.Elements(Nombre_alea) :=  Occurences.Elements(Nombre_alea) + 1;
        end loop; 
        
        -- recherche du min et du max 
        Min := Occurences.Elements(1);
        Max := Occurences.Elements(1);
        for i in 1..Borne loop
            if Min > Occurences.Elements(i) then
                Min := Occurences.Elements(i); 
            elsif Max < Occurences.Elements(i) then
                Max := Occurences.Elements(i);
            end if;  
        end loop; 
        ...
        Moyenne := 0.0;
        for i in 1..Borne loop
            Moyenne := Moyenne + Float (Occurences.Elements(i)); 
        end loop; 
