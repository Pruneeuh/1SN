...
procedure Tableau_Supprimer is
    ...
    -- Objectif : Supprimer tout les tous les entiers strictement négatif et tout les entiers strictement supérieur à 20
    -- Paramètres : 
    --      Tab : tableau à modifier
    procedure Supprimer(Tab : in out T_Tableau) is 
        
        stockage : integer ; --variable de stockage pour échange de 2 valeurs 
        taille_effect : integer ; -- taille effective souhaitée (mais pas encore modifée) 
    begin 
        taille_effect := Tab.Taille; 
        for I in 1..Tab.Taille loop
            while (Tab.Elements(I) < 0 or Tab.Elements(I) > 20) and I<=taille_effect loop
                stockage := Tab.Elements(I) ;
                Tab.Elements(I) := Tab.Elements(taille_effect);
                Tab.Elements(taille_effect) := stockage; 
                taille_effect := taille_effect - 1; 
           end loop; 
        end loop; 
        Tab.Taille := taille_effect; 
    end Supprimer; 
