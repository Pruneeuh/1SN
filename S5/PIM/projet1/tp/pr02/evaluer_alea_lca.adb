...
procedure Evaluer_Alea_LCA is
	...
	is
        ...
        package LCA_Alea is new LCA(T_Cle => Integer, T_Valeur => Integer);
        use LCA_Alea;
    ...
    begin
		...
		for i in 1..Taille loop
			...
			if Cle_Presente(Sda, Alea) then 
				Valeur := La_Valeur(Sda,Alea); 
				Enregistrer(Sda,Alea, Valeur + 1); 
			else 
                Enregistrer(Sda,Alea,1); 
             end if;
		end loop; 
        ...
        for i in 2..Borne loop
			Valeur := LCA_Alea.La_Valeur(Sda,i);
			if Valeur < min then 
				min := Valeur; 
