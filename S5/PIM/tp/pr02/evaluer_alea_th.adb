...
procedure Evaluer_Alea_TH is
	...
	is
		...
		function Hachage_Alea(Cle : Integer) return Integer is 
		begin 
			return Cle; 
		end Hachage_Alea; 
		
		package TH_Alea is new TH(Capacite => Borne+1, T_Cle_TH => Integer, T_Valeur_TH => Integer, Hachage => Hachage_Alea);
        use TH_Alea;
        
		Sda : T_TH; 
		Alea : Integer; -- Valeur tirée aléatoirement 
		Valeur : Integer; -- Stockage de la fréquence actuelle associée à la Clé tirée 
	...
	begin
		...
		for i in 1..Taille loop
			Get_Random_Number(Alea); 
			if Cle_Presente_TH(Sda, Alea) then 
				Valeur := La_Valeur_TH(Sda,Alea); 
				Enregistrer_TH(Sda,Alea, Valeur + 1); 
			else 
                Enregistrer_TH(Sda,Alea,1); 
            end if;
            
		end loop; 
		
		-- Rechercher le minimum et le maximum de chaque fréquence 
		min := Taille ; 
        max := 0 ; 
        
		for i in 1..Borne loop
			Valeur := La_Valeur_TH(Sda,i);
			if Valeur < min then 
				min := Valeur; 
...
exception
    ...
    when others => Put("les données doivent être deux entiers strictement positifs");
