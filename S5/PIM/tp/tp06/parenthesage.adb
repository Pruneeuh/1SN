...
procedure Parenthesage is
    ...
    is
    I : Integer ; -- indice de parcourt de la chaine de caractère
    begin
        I:=Meule'First;
        while I <= Meule'Last and then Meule(I) /= Aiguille loop
            I := I + 1;
        end loop;
        return I;
    end Index;
    ...
    procedure Verifier_Parenthesage (Chaine: in String ; Correct : out Boolean ; Indice_Erreur : out Integer) is
        ...
        Fermants : Constant String := ")]}";
         ...
         package PileSymbole is
            new Piles(Chaine'Last, Character);
        use PileSymbole;

        package PileIndice is
            new Piles(Chaine'Last, Integer);
        use PileIndice;

        Pile_Ouvrants : PileSymbole.T_Pile;
        Pile_Indice : PileIndice.T_Pile;
    ...
    begin
        ...
        while I<=Chaine'Last and Correct loop
            ...
            -- Traiter cas parenthèse ouvrante
            if Chaine(I) = '(' or Chaine(I) = '{' or Chaine(I) = '[' then
                Empiler(Pile_Ouvrants, Chaine(I));
                Empiler(Pile_Indice, I);

            -- Traiter cas parenthèse fermante
            elsif Chaine(I) = ')' or Chaine(I) = ']' or Chaine(I) = '}' then
