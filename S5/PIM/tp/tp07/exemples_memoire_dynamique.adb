...
procedure Exemples_Memoire_Dynamique is
	...
	begin
		...
		-- XXX Quelle est la valeur de Ptr2.all ? 5
		Put_Line ("Ptr2.all = " & Integer'Image (Ptr2.all));

		Ptr2.all := 7;
		pragma Assert (Ptr2.all = 7);

		-- XXX A-t-on le droit de manipuler Ptr2.all ? Non il recupère les données de l'adresse de Ptr1
		-- XXX Que se passe-t-il si on exécute le programme avec valgrind ? il y a 3 erreurs
		--
	...
	begin
		...
		Ptr.all := 111;
		SP (Ptr);
		-- XXX Quelle est la valeur de Ptr.all ? 123 - execution de SP(Ptr) qui le modifie
	...
	begin
		...
		-- d'appeler Detruire dessus ? OUI car mémoire auto libérée es juste celle de la première cellule, celle allouée dynamiquement non libérer | tjs détruire si mémoire allouée dynamiquement
