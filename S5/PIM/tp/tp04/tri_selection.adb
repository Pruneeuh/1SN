...
procedure Tri_Selection is
    ...
    -- Objectif : Trier un tableau suivant l'algorithme du tri par sélection
    -- Paramètres : 
    --      Tab : tableau à trier

    procedure Trier(Tab : in out T_Tableau) is 
        Ind_Min : Integer; -- Indice du minimum 
        Stockage : Integer; -- variable pour échanger deux valeurs 
    ...
    begin 
        for i in 1..Tab.Taille loop
            ...
            for j in i..Tab.Taille loop     
                if Tab.Elements(j) < Tab.Elements(Ind_Min) then
                    Ind_Min := j; 
                else 
                    null; 
                end if; 
           ...
           -- Echanger les éléments 
            ...
            Tab.Elements(i) := Tab.Elements(Ind_Min);
            Tab.Elements(Ind_Min) := Stockage;
