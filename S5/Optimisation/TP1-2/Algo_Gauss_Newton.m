...
% TO DO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    beta = beta0;
    norm_grad_f_beta = norm(J_residu(beta));
    f_beta = residu;
    norm_delta = 0;
    nb_it = 0;
    exitflag = 0;

    while (exitflag == 0) | (f_beta~=0)
        norm_grad_f_beta = norm(J_residu((residu(beta)).' + (J_residu(beta).')*residu(beta)).');
        delta = - (norm_grad_f_beta)^(-1)*J_residu(beta)'*residu(beta);
        beta = delta + beta ; 
        nb_it = nb_it + 1; 
