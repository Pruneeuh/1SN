...
% Lancement de l'exercice_2 :
exercice_2;

% Codage de Huffman de l'image d'origine :
[I_encodee,dictionnaire] = encodage_image(I); % FONCTION A COMPLETER
...
coeff_compression_avant_decorrelation = coefficient_compression(I(:),I_encodee); % FONCTION A COMPLETER

% Affichage du nombre de bits par symbole pour l'image d'origine :
figure('Position',[0.05*L,0.1*H,0.9*L,0.3*H],'Color',[0.7 0.75 0.85]);
subplot(1,2,1)
...
[I_encodee,dictionnaire] = encodage_image(I_decorrelee); % FONCTION A COMPLETER
nb_symboles = size(dictionnaire,1);
liste_symboles = zeros(nb_symboles,1);
longueur_symboles = zeros(nb_symboles,1);
for k = 1:nb_symboles
...
coeff_compression_apres_decorrelation = coefficient_compression(I_decorrelee(:),I_encodee); % FONCTION A COMPLETER
