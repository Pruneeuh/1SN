...
for indice_bruit=1:length(tab_Eb_N0_dB)
    ...
    TES_conv=0;          %Initialisation du TES BPSK pour le cumul
    TEB_conv=0;          %Initialisation du TEB BPSK pour le cumul
    ...
    while(nb_erreurs<1000)
        ...
        %Mapping convolutif
        symboles_conv= codage_convolutif(bits,g1,g2);   

        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %SURECHANTILLONNAGE
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        somme_Diracs_ponderes_conv=kron(symboles_conv,[1 zeros(1,Ns-1)]);
        ...
        %Filtrage de mise en forme
        Signal_emis_conv=filter(h,1,somme_Diracs_ponderes_conv);
        ...
        P_signal=mean(abs(Signal_emis_conv).^2);% A COMPLETER         
        ...
        Bruit=sqrt(P_bruit)*randn(1,length(Signal_emis_conv))+randn(1,length(Signal_emis_conv))*1i;% A COMPLETER             
        %Ajout du bruit canal au signal émis => signal à l'entrée du récepteur
        Signal_recu_conv=Signal_emis_conv+Bruit;
        ...
        Signal_recu_filtre_conv=filter(hr,1,Signal_recu_conv);
        ...
        Signal_echantillonne_conv=Signal_recu_filtre_conv(n0:Ns:end);
        
        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %DECISIONS SUR LES SYMBOLES 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        symboles_recus_decimaux_conv=(Signal_echantillonne_conv + 1)./2;     % A COMPLETER
               
        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %CALCUL DU TAUX D'ERREUR SYMBOLE CUMULE
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        TES_conv=TES_conv+sum(symboles_recus_decimaux_conv ~= symboles_conv )/(N/log2(M));      % A COMPLETER
        
        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %DEMAPPING
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %% faire le décodage de vitrisky (ou a peu près)
        bits_recus_conv=int2bit(symboles_recus_decimaux_BPSK', log2(M))';    % A COMPLETER
                
        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %CALCUL DU TAUX D'ERREUR BINAIRE CUMULE
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        TEB_conv=TEB_conv+sum(bits ~= bits_recus_conv)/N;    % A COMPLETER
        
        %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %CUMUL DU NOMBRE D'ERREURS ET NOMBRE DE CUMUL REALISES
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        nb_erreurs=nb_erreurs+sum(bits ~= bits_recus_conv);
        nb_cumul=nb_cumul+1;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    TES_simule_conv(indice_bruit)=TES_conv/nb_cumul; 
    TEB_simule_conv(indice_bruit)=TEB_conv/nb_cumul; 
    ...
    %MODULATION BPSK
    figure
    plot(real(Signal_echantillonne_conv),imag(Signal_echantillonne_conv),LineStyle='none',Marker='+');     % A COMPLETER
    xlabel('a_k')
    ylabel('b_k')
    title(['Tracé de la constellation en sortie du filtre de réception (conv) pour E_b/N_0 = ' num2str(Eb_N0_dB) 'dB'])
    grid on
...
hold on
semilogy(tab_Eb_N0_dB, TES_simule_conv,'b-o')
legend('TES théorique BPSK','TES simulé conv')
xlabel('E_b/N_0 (dB)')
ylabel('TES')
...
hold on
semilogy(tab_Eb_N0_dB, TEB_simule_conv,'b-o')
legend('TEB théorique BPSK','TEB simulé conv')
xlabel('E_b/N_0 (dB)')
ylabel('TEB')
