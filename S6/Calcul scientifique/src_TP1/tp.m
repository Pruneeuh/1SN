...
for i = 1:k
  ...
  cond_mat(1,i)=cond(Qcgs);

  po(1,i) = norm(eye(n)-Qcgs'*Qcgs);
  Qcgs_bis = cgs(Qcgs); 
  po(2,i) = norm(eye(n)-Qcgs_bis'*Qcgs_bis);
  % Perte d'orthogonalite avec algorithme modifie
  Qmgs = mgs(A);

  cond_mat(2,i)=cond(Qmgs);

  po(3,i) = norm(eye(n)-Qmgs'*Qmgs);
  Qmgs_bis = mgs(Qmgs);
  po(4,i) = norm(eye(n)-Qmgs_bis'*Qmgs_bis);
