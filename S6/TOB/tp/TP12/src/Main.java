...
public class Main {
	...
	private static void repondreQuestions(Reader in) {
		...
		for (final PointDeVente pdv : iterable){
		...
		int nbServices = 0;
			Set<String> services;
		for (final PointDeVente pdv : iterable){
			Iterator serviceActuel = pdv.getServices().iterator(); 
			//Set<String> serviceActuel = pdv.getServices(); 
			while (serviceActuel.hasNext()){
				String la = serviceActuel.next();
			}
			if (!services.contains(serviceTest)){
				boolean v = services.add(serviceTest);
			}
