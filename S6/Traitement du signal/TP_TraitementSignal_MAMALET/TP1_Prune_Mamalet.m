...
SNR=10;        %SNR souhaité en dB pour le cosinus bruité (Signal to Noise Ratio)
...
semilogy(echelle_frequentielle,mod_X);
grid
...
semilogy(echelle_frequentielle,mod_X_ZP);
grid
...
X_shift = fftshift(X);
X_ZP_shift = fftshift(X_ZP);
figure
echelle_frequentielle=[-Fe/2:Fe/(N-1):Fe/2];
semilogy(echelle_frequentielle,abs(X_shift),'b');    %Tracé en bleu : 'b'
hold on
echelle_frequentielle=[-Fe/2:Fe/(Nfft-1):Fe/2];
semilogy(echelle_frequentielle,abs(X_ZP_shift),'r'); %Tracé en rouge : 'r'
grid
...
X_ZP_blackman_shift = fftshift(X_ZP_blackman);
X_ZP_hamming_shift = fftshift(X_ZP_hamming);
figure
echelle_frequentielle = [-Fe/2:Fe/(Nfft-1):Fe/2];
semilogy(echelle_frequentielle,abs(X_ZP_blackman_shift),'r'); %Tracé en rouge : 'r'
hold on
semilogy(echelle_frequentielle,abs(X_ZP_hamming_shift),'b'); 
hold on 
semilogy(echelle_frequentielle,abs(X_ZP_shift),'g'); 
grid
legend('blackman','hamming','sans fenetre')
xlabel('Fréquence (Hz)')
ylabel('|TFD|')
title(['Tracé du module de la TFD d''un cosinus numérique de fréquence ' num2str(f0) 'Hz'])
...
Sx_periodogramme=abs(fft(x,Nfft)).^2; %carré de |TFD|
...
figure
echelle_tau=[-(N-1):(N-1)]*Te;
plot(echelle_tau,Rx)
grid
xlabel('\tau (s)')
ylabel('R_x(\tau)')
title('Tracé de la fonction d''autocorrélation du cosinus bruité');
%Un rapport signal sur bruit de 0dB correspond à un bruit et un signal de meme puissance 
% Si on connait la puissance du bruit on peut retrouver 
% le SNR = (Rx(0) - Pbruit)/Pbruit
%
